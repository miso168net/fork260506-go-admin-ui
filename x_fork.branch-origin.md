...
| 項目 | 內容 |
|---|---|
| 此 repo | `miso168net/fork260506-go-admin-ui` |
| 原始專案(推測) | `go-admin-team/go-admin-ui` |
| Fork 用途 | 個人學習與實驗用 fork |
| Fork 建立日 | 2026-05-06 |
| `main` 來源分支 | `dev-to-vue3` |
| 建立時的來源 HEAD | `3a96323` — fix: 更新 chart-card 组件的插槽语法以支持新模板(2026-01-31) |
| 原本的 default branch | `master` |
| 改用 main 的原因 | 專案正在從 Vue 2 遷移到 Vue 3,`master` 落後 1 年 5 個月,`main` 跟隨遷移分支。 |

## 歷史說明

- 原本的 default branch `master` 完整保留,沒有刪除或修改。
- `main` 是新建的分支,從 `dev-to-vue3` 拉出,目的是對齊本專案實際的開發主線。
