---
source_file: "src/views/dev-tools/gen/index.vue"
type: "code"
community: "Utility Functions"
location: "L337"
tags:
  - graphify/code
  - graphify/EXTRACTED
  - community/Utility_Functions
---

# handleDelete()

## Connections
- [[delConfig()]] - `calls` [INFERRED]
- [[delPost()]] - `calls` [INFERRED]
- [[delSysLoginlog()]] - `calls` [INFERRED]
- [[delSysOperlog()]] - `calls` [INFERRED]
- [[delTable()]] - `calls` [INFERRED]
- [[delType()]] - `calls` [INFERRED]
- [[index.vue_30]] - `contains` [EXTRACTED]
- [[index.vue_32]] - `contains` [EXTRACTED]
- [[index.vue_34]] - `contains` [EXTRACTED]
- [[index.vue_36]] - `contains` [EXTRACTED]
- [[index.vue_37]] - `contains` [EXTRACTED]
- [[index.vue_44]] - `contains` [EXTRACTED]

#graphify/code #graphify/EXTRACTED #community/Utility_Functions